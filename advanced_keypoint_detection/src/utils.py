...
def valid_keypoints_plot(image, outputs, orig_keypoints, epoch):
    ...
    """
    # detach the image, keypoints, and output tensors
    # from the GPU to CPU
...
def dataset_keypoints_plot(data):
    """
    This function shows the image faces and keypoint plots that 
    the model will actually see. This is a good way to validate
    that our dataset is in fact correct and the faces align 
    with the keypoint features. The plot will be shown just 
    before training starts. Press 'q' to wuit the plot and start 
    training.
    """
    ...
    for i in range(9):
        ...
        # convert the image to an array beforehand
        img = np.array(img, dtype='float32')
        img = np.transpose(img, (1, 2, 0))
