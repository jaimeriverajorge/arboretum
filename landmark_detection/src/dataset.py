...
class FaceKeypointDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # not all of the images are 224 x 224, so if the image
        # has been resized, its corresponding keypoints must be
        # resized as well
...
training_samples, valid_samples = train_test_split(
    f"{config.ROOT_PATH}/three_training.csv", config.TEST_SPLIT)

train_data = FaceKeypointDataset(
    training_samples, f"{config.ROOT_PATH}/training")
valid_data = FaceKeypointDataset(
    valid_samples, f"{config.ROOT_PATH}/training")
