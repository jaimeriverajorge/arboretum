# script to train and validate our neural network
# on the facial keypoint dataset
...
# initialize optimizer, pass model parameters
# and learning rate into it
optimizer = optim.Adam(model.parameters(), lr=config.LR)
# need a loss function that is good for regression
# such as MSELoss
...
def validate(model, dataloader, data, epoch):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader), total=num_batches):
            ...
            # plot the predicted validation keypionts after every
...
# execute the fit() and validate() functions and store
# the loss values in respective lists
train_loss = []
val_loss = []
...
# plot the loss graphs and save the trained neural network
